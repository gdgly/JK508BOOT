...
uint8_t File_IAP(void)
{
	...
	u16 i=0;
	u32 wr_state=0;
	static uint32_t fsize,wrtime,lastsize;
	static u8 res;
	static u8 iapname[64];
	char buf[30];
	u8 iapprocess;
	...
	strcpy((char *)iapname,"UPDATE.BIN");
	res = CH376FileOpenPath(iapname);
	if(res != 0x14)
	{
		goto updateerror;
	}
	fsize = CH376GetFileSize();
	wrtime = fsize/2048;
	lastsize = fsize%2048;
	LCD_SetColors(LCD_COLOR_RED,LCD_COLOR_BACK);
	LCD_DisplayStringLine(130,100, "正在升级,请勿关闭电源");
...
//	{
		do{
			res = CH376ByteRead( filedatabuf, 2048, &sizecount );
//		 {
//     //  LCD_UsrLog("f_read->updata.bin->error.\n");
//			 goto updateerror;
//     }
		
		 wr_state=IAP_Flash_Write(USER_FLASH_FIRST_PAGE_ADDRESS+i*2048,filedatabuf,sizecount/4);
		 iapprocess = (u8)((float)(i+1)*2048.0/(float)fsize * 100);
		 sprintf(buf,"%d",iapprocess);
		 LCD_SetColors(LCD_COLOR_GREEN,LCD_COLOR_BACK);
		 LCD_DisplayStringLine(160,112,((uint8_t *)buf));
		 LCD_DisplayStringLine(160,160,"%");
		 if(0!=wr_state) 
		 {
...
//	  goto updateerror;
//  }
//	sprintf(buf,"%d",sizecount);
//	DISP_TEMP_L(200,200,(uint8_t *)buf,0);
	 LCD_SetColors(LCD_COLOR_GREEN,LCD_COLOR_BACK);
	LCD_DisplayStringLine(190,100,"升级完成,请重启仪器");
	res = CH376FileClose(FALSE);
	return 0;
	...
	updateerror:
	 LCD_SetColors(LCD_COLOR_RED,LCD_COLOR_BACK);
	 LCD_DisplayStringLine(130,100,"未检测到升级文件");
...
//		watch = GPIO_ReadInputDataBit(TOUCH_YPLUS_GPIO_PORT,TOUCH_YPLUS_GPIO_PIN);
		...
		{
			PowerOffHandle();
		}else if(page_flag != poweroff){
			LCD_SetColors(LCD_COLOR_RED,LCD_COLOR_BACK);
			LCD_DisplayStringLine(100,100, "请插入U盘");
			
//			RTC_TimeAndDate_Show();
			if(udisk_scan() == UDISK_READY)
			{
				File_IAP();
//				write_test();
			}
